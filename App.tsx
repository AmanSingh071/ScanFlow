...
type Doc={id:string;name:string;pages:Page[];createdAt:number;favorite:boolean;folder:string;pdfUri?:string;sourceUri?:string;fileType?:"pdf"|"text";note?:string};
...
export default function App(){
 ...
 <Modal visible={!!menu} transparent animationType="slide"><View style={s.overlay}><View style={[s.sheet,{backgroundColor:c.card}]}>{menu&&<><Text style={[s.sheetTitle,{color:c.text}]} numberOfLines={1}>{menu.name}</Text><TouchableOpacity style={s.menuRow} onPress={()=>setRenaming(true)}><Text style={[s.menuText,{color:c.text}]}>✏️ Rename</Text></TouchableOpacity><TouchableOpacity style={s.menuRow} onPress={()=>toggleFav(menu.id)}><Text style={[s.menuText,{color:c.text}]}>{menu.favorite?"☆ Remove favorite":"⭐ Add to favorites"}</Text></TouchableOpacity><TouchableOpacity style={s.menuRow} onPress={()=>exportPdf(menu)}><Text style={[s.menuText,{color:c.text}]}>📄 Create / share PDF</Text></TouchableOpacity><TouchableOpacity style={s.menuRow} onPress={()=>moveFolder(menu)}><Text style={[s.menuText,{color:c.text}]}>📂 Move to folder</Text></TouchableOpacity><TouchableOpacity style={s.menuRow} onPress={()=>openNote(menu)}><Text style={[s.menuText,{color:c.text}]}>📝 Add / edit note</Text></TouchableOpacity>{menu.note?<TouchableOpacity style={s.menuRow} onPress={()=>copyNote(menu)}><Text style={[s.menuText,{color:c.text}]}>📋 Copy text / note</Text></TouchableOpacity>:null}<TouchableOpacity style={s.menuRow} onPress={()=>trashDoc(menu)}><Text style={[s.menuText,{color:c.danger}]}>🗑️ Move to trash</Text></TouchableOpacity><TouchableOpacity onPress={()=>setMenu(null)} style={s.cancel}><Text style={{color:c.muted}}>Close</Text></TouchableOpacity></>}</View></View></Modal>
