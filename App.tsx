...
type Doc={id:string;name:string;pages:Page[];createdAt:number;favorite:boolean;folder:string;pdfUri?:string;sourceUri?:string;fileType?:"pdf"|"text";note?:string;signature?:string;annotation?:string;tags?:string[];lastOpened?:number};
...
export default function App(){
 ...
 function toggleSelect(id:string){setSelected(a=>a.includes(id)?a.filter(x=>x!==id):[...a,id])}
 function bulkDelete(){if(!selected.length)return;Alert.alert("Delete documents",`Delete ${selected.length} selected document(s)?`,[{text:"Cancel",style:"cancel"},{text:"Delete",style:"destructive",onPress:()=>{setDocs(d=>d.filter(x=>!selected.includes(x.id)));setSelected([]);setSelectMode(false)}}])}
 function bulkFavorite(){setDocs(d=>d.map(x=>selected.includes(x.id)?{...x,favorite:true}:x));setSelected([]);setSelectMode(false)}
 function saveTags(){if(!menu)return;const next=tag.split(",").map(z=>z.trim()).filter(Boolean);setDocs(d=>d.map(x=>x.id===menu.id?{...x,tags:next}:x));setTagModal(false);setMenu(null)}
 async function downloadResult(url:string,name:string){
  const target=FileSystem.cacheDirectory+name;
  const r=await FileSystem.downloadAsync(url,target);
 ...
 const library=<>
  ...
  renderItem={({item})=><TouchableOpacity onPress={()=>shareDoc(item)} onLongPress={()=>{setMenu(item);setRename(item.name)}} style={[s.doc,{backgroundColor:c.card}]}>
   ...
   <View style={{flex:1,marginLeft:12}}><View style={s.row}><Text numberOfLines={1} style={[s.docName,{color:c.text}]}>{item.name}</Text>{item.favorite&&<Text>⭐</Text>}</View><Text style={{color:c.muted,fontSize:12}}>{item.pages.length?item.pages.length+" pages":"PDF"} • {item.folder}</Text>{(item.tags||[]).length?<Text style={{color:c.accent,fontSize:11,marginTop:4}}>{(item.tags||[]).map(t=>"#"+t).join(" ")}</Text>:null}<Text style={{color:c.muted,fontSize:11,marginTop:3}}>{new Date(item.createdAt).toLocaleDateString()}</Text></View>
